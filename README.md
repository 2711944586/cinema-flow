...
![CinemaFlow Dashboard](docs/screenshots/dashboard.png)
...
| 页面 | 路径 | 说明 |
| --- | --- | --- |
| Dashboard | `/dashboard` | 片库状态、最近浏览、最近添加与快速入口 |
| Movies | `/movies` | 搜索、筛选、排序、分页、表格/网格切换 |
| Genre Movies | `/movies/genre/:genre` | 类型路由与列表筛选同步 |
| Movie Detail | `/movies/:id/info` | 电影详情、收藏、已看状态、相邻电影与推荐 |
| Cast | `/movies/:id/cast` | 演员表子页面 |
...
| Random | `/random` | 随机选片与抽选历史 |
| Compare | `/compare` | 两部电影并列对比 |
| Calendar | `/calendar` | 观影日志月历、年度热度与当日记录 |
| Reviews | `/reviews` | 影评墙、录入、筛选、排序与分页 |
| Watch Plans | `/watch-plans` | 待看片单、优先级、排期和状态管理 |
| Watch Logs | `/watch-logs` | 日期、地点、陪伴对象、评分和情绪标签 |
| Smart Picks | `/smart-picks` | 基于预设条件推荐下一部电影 |
| Director Atlas | `/director-atlas` | 按导演聚合作品数量、均分和观看进度 |
| Mood Atlas | `/mood-atlas` | 按情绪标签聚合观影记录 |
| Marathon | `/marathon` | 按时长预算组合连看片单 |
| Taste DNA | `/taste-dna` | 已看、收藏、评分、影评与日志偏好画像 |
| Scene Board | `/scene-board` | 按视觉气质重组片库 |
| Archive Health | `/archive-health` | 审计媒体 URL、简介、演员表和关键字段 |
| About | `/about` | 服务状态、技术栈、日志、数据管理 |

## 新增独立功能

`Taste DNA` 汇总已看、收藏、个人评分、影评和观影日志，展示类型、导演、语言、年代与偏好盲区。

![Taste DNA](docs/screenshots/taste-dna.png)

`Scene Board` 按视觉气质组织电影，适合主题策展和快速选片。

![Scene Board](docs/screenshots/scene-board.png)

`Archive Health` 检查海报、背景图、简介、演员表、语言和关键字段，给出优先修复清单。
...
| 截图 | 页面说明 |
| --- | --- |
| `docs/screenshots/dashboard.png` | Dashboard 全局概览 |
| `docs/screenshots/movies.png` | Movies 标准列表 |
...
| `docs/screenshots/scene-board.png` | 氛围策展板 |
| `docs/screenshots/archive-health.png` | 片库质量审计 |
| `docs/screenshots/about.png` | About 与数据管理 |
